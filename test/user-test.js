...
describe('USER TEST   overAll test', () => {
  ...
  it('expected to sign-up a User', async () => {
  ...
  it('expected to throw error on failed input validation', async () => {
    const res = await chai.request(server)
      .post('/api/v1/auth/signup')
  ...
  it('expected to sign-in an existing User', async () => {
    const res = await chai.request(server)
      .post('/api/v1/auth/signin')
      .send(userItems.userLogin);
    token2 = res.body.Data.token;
    token2 = token2.replace(/^"(.*)"$/, '$1');
  ...
  it('get all the users in the application', async () => {
    const res = await chai.request(server)
      .get('/api/v1/users')
      .set('Authorization', token2);
    expect(res, 'must have the status code 200').to.have.status(200);
  });
});
